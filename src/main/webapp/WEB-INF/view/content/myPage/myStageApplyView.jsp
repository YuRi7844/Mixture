...
$(document).ready(function(){
	$(".cancelBtn").on("click", function(){
		var success = confirm("예약을 취소하시겠습니까?");
		if(success){
			$.ajax({
				"url":"${initParam.rootPath}/cancelStageReservation2.do",
				"type":"post",
				"data":{
		...
		<tr style="height: 50px;">
			...
			<c:choose>
				<c:when test="${item.rentalStateCode eq 0 }">
					<td style="font-weight: bold;">거절됨</td>
				</c:when>
				<c:when test="${item.rentalStateCode eq 2 }">
					<td style="font-weight: bold;">수락됨</td>
				</c:when>
				<c:when test="${item.rentalStateCode eq 3 }">
					<td style="font-weight: bold;">취소됨</td>
				</c:when>
				<c:otherwise>
					<td>
						<button type="button" class="btn btn-default cancelBtn">예약 취소</button>
					</td>
				</c:otherwise>
			</c:choose>
		</tr>
